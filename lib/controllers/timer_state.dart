...
class TimerState extends GetxController {
  ...
  void periodicUpdateDisplayString() {
    ...
    t = Timer.periodic(Duration(milliseconds: gameController.hizZaman), (t) {
      displayString.value = updateDisplayString();
  ...
  String updateDisplayString() {
    //ledleri dondur
    gameController.tickArtir();
    print(gameController.board[0]);
    ...
    if (_tsw.elapsed.inMinutes >= startingMin &&
        _tsw.elapsed.inSeconds >= startSec) {
      setTimerToFinished();
      str = "00:00";
    } else {
      // 24.59
      ...
      // value we set
      str +=
          "${((startSec - _tsw.elapsed.inSeconds) % 60).toString().padLeft(2, "0")}";
    }
